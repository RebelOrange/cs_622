...
class DataManager:
    ...
    def LoadTrainAndTestData(self, folderName: str = None, csvFileName: str = None, numFiles: int = None,
                             classFilter: list[str] = None, split: float = 0.8):
        ...
        for i in range(k):
            ...
            if i == foldIndex:
                for label in self.data["label"].unique():
                    self.KTestData = pd.concat(
                        [self.KTestData, self.TrainingData[self.TrainingData["label"] == label].iloc[mask]])
            else:
                for label in self.data["label"].unique():
                    self.KTrainingData = pd.concat(
                        [self.KTrainingData, self.TrainingData[self.TrainingData["label"] == label].iloc[mask]])
...
if __name__ == "__main__":
    ...
    t.start()
    dm.LoadTrainAndTestData(folderName=currentFolder + "//..//data//", csvFileName="Training_set.csv", numFiles=100,
                            classFilter=classes)
