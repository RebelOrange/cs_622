...
from PlotUltilities import plotConfusionMatrix, plotConfigMatrix, basePlot, plotRocCurve, computeModelRoc, \
    plotModelCompareRoc
...
class ModelEvaluator:
    ...
    def evaluateModelVariants(self, modelClass, prefix, trainingData, numClasses=None, batchSize=32, numEpochs=5,
                              ...
                              saveBest=False, loadBest=False, savePath=None, saveCsv=False, modelType=None,
                              variant=None, targetAccuracy=None):
        ...
        if modelClass == NNModel:
            baseModel = modelClass(numClasses=numClasses, modelDir=modelDir, modelType=modelType or "EfficientNet",
                                   variant=variant or 'b0')
        else:
        ...
        for i, config in enumerate(configs):
            ...
            try:
                ...
                if modelClass == NNModel:
                    model = modelClass(numClasses=numClasses, modelDir=modelDir, modelType=modelType or "EfficientNet",
                                       variant=variant or 'b0', **mappedConfig)
                else:
    ...
    def evaluateArchitectures(self, trainingData, architectures, modelDir="models", numEpochs=5, batchSize=32,
                              maxConfigs=2,
                              trainRatio=0.7, valRatio=0.15, testRatio=0.15, saveBest=False, loadBest=False,
                              outputDir=None, targetAccuracy=None):
        ...
        for arch in architectures:
            ...
            if outputDir:
                savePath = os.path.join(outputDir,
                                        f"{prefix}_{modelType or ''}_{variant or ''}_results".replace('__', '_'))
